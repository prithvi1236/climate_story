...
dataset_path = 'methane_hist_emissions.csv'
...
def plot_graph():
    ...
    if country not in df['Country'].values:
        return jsonify({"error": "Country not found in the dataset"
                        ,"Country":country
                        }), 404
    
    # Filter the row for the inputted country
    country_data = df[df['Country'] == country].iloc[0, 1:]  # Exclude the country column
...
def return_image():
    ...
    if not country or country not in df['Country'].values:
